...
define(["avalon",
    ...
    avalon.component("aoyou:pager", {
        ...
        showPageNumButtons:true,//@config {Boolean} 是否显示页码部分包括省略符号
        alwaysShowNext: false, //@config {Boolean} 总是显示向后按钮
        alwaysShowPrev: false, //@config {Boolean} 总是显示向前按钮
        alwaysShowFirst:false,//@config {Boolean} 总是显示回到第一项按钮
        alwasyShowLast:false,//@config {Boolean} 总是显示最后一项按钮
        showFirstOmit: false,
        showLastOmit: false,
        ...
        //===============需要重写的方法与事件回调============
        /**
         * @config {Function} 页面跳转时触发的函数,如果当前链接处于不可以点状态(aoyou-state-disabled),是不会触发的
         * @param {Event} e
    ...
    var widget = avalon.components["aoyou:pager"]
    widget.regionals = {}
