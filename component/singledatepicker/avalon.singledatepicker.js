...
define(['avalon', '../base/normalThings','css!./avalon.singledatepicker.css'], function(avalon) {
    var lf = '<div ms-click="datepickerClickFunc" ms-visible="showpk" class="singledatepicker" ms-css-width="cpwidth"><span class="trangle"></span>'+
        '            <div class="singledatepicker-tt">'+
        '                <span class="icon-arr-left year" ms-click="_linkageDatepicker(\'prev\',\'year\')"></span>'+
        '                <span class="icon-arr-left month" ms-click="_linkageDatepicker(\'prev\',\'month\')"></span>'+
        '                <span class="icon-arr-center">{{dataTable_title1}}&nbsp;{{useY}}</span>'+
        '                <span class="icon-arr-right year" ms-click="_linkageDatepicker(\'next\',\'year\')"></span>'+
        '                <span class="icon-arr-right month" ms-click="_linkageDatepicker(\'next\',\'month\')"></span>'+
        '            </div>'+
        '            <table class="singledatepicker-table">'+
        '                <thead><tr>'+
        '                    <th class="live">日</th>'+
        '                    <th>一</th>'+
        '                    <th>二</th>'+
        ...
        '                    <td ms-hover="{{stylehover}}" ms-class-1="{{el.dayStyle}}" ms-class-2="{{($index==0||$index==6)&&(el.dayStyle != \'out\'&&el.dayStyle != \'nouse\')?\'live\':\'\'}}" ms-class-3="{{($index==0||$index==6)&&(el.dayStyle==\'out\'||el.dayStyle==\'nouse\')?\'lived\':\'\'}}" ms-class-4="{{el.dayStr==nowdateStr?\'today\':\'\'}}" ms-repeat="dateTable_1[$index]" ms-on-click="_checkDateOfthis($event,el.dayStr,$index,$outer.$index,el.arrname,1,el.originalIdx)" ms-attr-innerIndex="$index" ms-attr-outerIndex="$outer.$index" ms-attr-datestr="el.dayStr" ms-attr-arrnameidx="1"><b ms-if="el.useday"></b><a href="javascript:;"><span class="basetxt">{{el.dayStr==nowdateStr?"今天":el.daynum}}</span><span class="orangetxt" ms-if="el.price">￥{{el.price}}起</span><i ms-attr-title="el.festival" class="festival" ms-if="el.festival"></i></a></td>'+
        '                </tr>'+
        '                <tr ms-visible="buttonshow" class="singledatepicker-buttons">'+
        '                    <td class="singledatepicker-buttons-content" colspan="7"><table><tr><td><span ms-click="_toToday()">今天</span></td><td><span ms-click="_closepicker()">关闭</span></td><td><span ms-click="_emptySelectedDates()">清空</span></td></tr></table></td>'+
        '                </tr>'+
        '            </table>'+
        '        </div>';
    ...
    _toToday:_interface,//到今天
    _emptySelectedDates:_interface,
    datepickerClickFunc:_interface,
    $template: lf,
    _tempelem:null,//记录外部绑定dom
    _tempshowFlag:true,//临时记录datepciker的显示状态
    $init: function(vm, el) {
        ...
        var deleteDates = {};
            ...
            vm.datepickerClickFunc = function(){
                if(avalon.isIE()){
            ...
            if(arrname){
                    ...
                    if(vm.aftercheckfunc != _interface){
                        vm.aftercheckfunc(vm.selectedDates);
                    }
                }
            }
            ...
            vm._dateinfo = function(m,y){
                vm.dateTable_1 = [];
                vm.dataTable_title1 = mChineseName[m];//1表头日期
            ...
            vm._emptySelectedDates = function(){
                ...
                vm.selectedDatesObj = {};
                vm._dateinfo(vm.useM,vm.useY);
                if(vm.afterClearfunc != _interface){
                    vm.afterClearfunc();
                }
            }
            vm._toToday = function(){
                vm.useM = CM;
                vm.useY = CY;
        ...
        var _elems = avalon.normalElementSetAttr('singledatepicker',vm.$id);
        if(_elems != undefined && _elems.length>0){
            vm._tempelem = _elems[0];
            if(vm.showpk != true){
                vm._tempshowFlag = false;
            }
            vm.showpk = false;
        }
    },
    $ready:function(vm,el){
        if(vm._tempelem != null){
            var styleattrs = {x:vm._tempelem.offsetLeft,y:vm._tempelem.offsetTop+8};
            if(styleattrs){
                var _targetEle = avalon.superGetElementeByClass(el,'singledatepicker')[0];
                if(_targetEle){
                    _targetEle.style.position = 'absolute';
                    _targetEle.style.left = styleattrs.x + 'px';
                    _targetEle.style.top = styleattrs.y + vm._tempelem.offsetHeight + 'px';
                    vm.showpk = vm._tempshowFlag;
                }
            }
        }
