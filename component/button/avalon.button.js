 ...
 */
define(["avalon",  "css!./avalon.button.css"], function (avalon) {
    var baseClasses = ["aoyou-button", "aoyou-widget", "aoyou-state-default"]
    ...
    function createButton(element, options) {
        ...
        element.tabIndex = -1
        if (corner) {
            buttonClasses.push("aoyou-corner-all")
        ...
        if (options.size) {
            buttonClasses.push("aoyou-button-" + options.size)
        }
        if (options.color) {
            buttonClasses.push("aoyou-button-" + options.color)
        }
        if (options.disabled) {
            buttonClasses.push("aoyou-state-disabled")
        }
        avalon(element).addClass(buttonClasses.join(" "))

        switch (options.type) {
            case "text":
                buttonText = "<span class='aoyou-button-text'>{{label|html}}</span>"
                break;
            case "labeledIcon":
                iconText = true
            case "icon":
                switch (options.iconPosition) {
                    case "left":
                        buttonText = "<i class='share-"+ options.icon +"'></i>" +
                            "<span class='aoyou-button-text aoyou-button-text-right"
                            + (!iconText ? " aoyou-button-text-hidden" : "") + "'>{{label|html}}</span>"
                        break;
                    case "right":
                        buttonText = "<span class='aoyou-button-text aoyou-button-text-left" +
                            (!iconText ? " aoyou-button-text-hidden" : "") + "'>{{label|html}}</span>"
                            + "<i class='share-"+ options.icon +"'></i>"
                        break;
                    case "left-right":
                        var iconArr = icons && icons.split("-") || ["", ""],
                            iconLeft = iconArr[0],
                            iconRight = iconArr[1]
                        buttonText = "<i class='share-"+ iconLeft +"'></i>" +
                            "<span class='aoyou-button-text aoyou-button-text-middle" +
                            (!iconText ? " aoyou-button-text-hidden" : "") + "'>{{label|html}}</span>" +
                            "<i class='share-"+ iconRight +"'></i>"
                        break;
                }
                break;
        }
    ...
    avalon.component("aoyou:button", {
        $init: function (options, element) {
            ...
            $element.bind("mousedown", function (event) {
                stop(event)
                $element.addClass("aoyou-state-active")
            })
            $element.bind("mouseup", function (event) {
                stop(event)
                $element.removeClass("aoyou-state-active")
            })
            $element.bind("blur", function () {
                $element.removeClass("aoyou-state-active")
                $element.removeClass("aoyou-state-focus")
            })
            $element.bind("focus", function () {
                $element.addClass("aoyou-state-focus")
            })
        },
    ...
    avalon.component("aoyou:buttonset", {
        data: [],
        $init: function (options, element) {
            ...
            data.forEach(function (button, index) {
                ...
                buttonStr += ">" + (button.text || "") + "</aoyou:button>"
                buttons += buttonStr
            })
            if (buttons) {
                options.$$template = function () {
        ...
        $childReady: function (options, element, e) {
            ...
            $button.removeClass("aoyou-corner-all")
        ...
        $ready: function (options, element) {
            var buttons = options.$buttons,
                elementClass = [],
                firstButtonClass = "aoyou-corner-left",
                lastButtonClass = "aoyou-corner-right",
                n = buttons.length,
            ...
            elementClass.push("aoyou-buttonset")
            avalon(buttons[0]).addClass("aoyou-button-first")
            if (n && buttonsetCorner) {

                if (direction === "vertical") {
                    firstButtonClass = "aoyou-corner-top"
                    lastButtonClass = "aoyou-corner-bottom"
                }
                avalon(buttons[0]).addClass(firstButtonClass)
                avalon(buttons[n - 1]).addClass(lastButtonClass)
            }
            if (direction === "vertical") {
                elementClass.push("aoyou-buttonset-vertical")
