...
define(["avalon", "text!./avalon.tab.html",
    ...
    "css!./avalon.tab.css",
    ], function (avalon, template, closeTpl) {
...
// 对模板进行转换
    ...
    avalon.component("aoyou:tabs", {
        ...
        menutop:0,
        menu:'',
        menurefresh:_interface,
        onmenurefresh:_interface,
        menuclose:_interface,
        onmenuclose:_interface,
        menucloseall:_interface,
        onmenucloseall:_interface,
        menucloseright:_interface,
        menucloseleft:_interface,
        menucloseother:_interface,
        slider: _interface,
        onSlider:_interface,
        afterRemove:_interface,
        onRightMouse: _interface,
        ...
        $init: function (vm, elem) {
            ...
            vm.activate = function (event, index, fix) {
                ...
                //判断鼠标点击事件,2为鼠标右键,弹出右键相应菜单
                if(event.button==2){
                    vm.onRightMouse.call(el,event,index,vm);
                    /*vm.menushow = true;
                    vm.menuleft = event.pageX-215;
                    vm.menutop = event.pageY-50;*/
                }
            }
            // 延迟切换效果
            ...
            vm.add = function (config) {
                var name = config.name || "home";
                var title = config.title || "Tab Tile"
                var content = config.content || "<div></div>"
                var thishref = config.href || ""
                var exsited = false
                var iconshow = config.iconshow || false
            ...
            vm.remove = function (e, index) {
                ...
                if (vm.tabs[index].disabled === true || vm.removable ===false || vm.tabs[index].removable === false) {
                    return
                }
                ...
                avalon.nextTick(function () {
                    vm.active = index
                    vm.afterRemove(vm.tabs[vm.active],vm.active);
                })
            ...
            vm.slider = function ($event, dir) {
                ...
                if (dir === "prev") {
                    ...
                    step = (vm.active>=vm.tabs.length-1)?vm.tabs.length-1:vm.active+1
                }

                vm.active = step
                vm.onSlider(vm.tabs[step],step);
                ...
                if (vm.sliderStep.indexOf("%") !== -1) {
                    vm._computedMarginLeft = vm.active * parseInt(vm.sliderStep, 10) * containerWidth * 0.01
                } else if (vm.sliderStep.indexOf("px") !== -1) {
                    vm._computedMarginLeft = vm.active * parseInt(vm.sliderStep, 10)
                } else {
            ...
            vm.menurefresh = function ($event) {
                vm.onmenurefresh($event,vm);
            }
            vm.menuclose = function ($event) {
                vm.remove($event,vm.active);
                vm.onmenuclose($event,vm);
            }
            vm.menucloseall = function ($event) {
                vm.tabs.splice(0,vm.tabs.length);
                vm.tabpanels.splice(0,vm.tabpanels.length);
                vm.onmenucloseall($event,vm);
            }
            vm.menucloseright = function ($event) {
                for(var i = vm.tabs.length;i > vm.active; i--){
                    vm.tabs.removeAt(i)
                    vm.tabpanels.removeAt(i)
                }
                vm.onmenuclose($event,vm);
            }
            vm.menucloseleft = function ($event) {
                var temp = 0;
                for(var i = vm.active-1;i>=0;i--){
                    vm.tabs.removeAt(i)
                    vm.tabpanels.removeAt(i)
                    temp++;
                }
                var index = index > temp ? index - temp : 0;
                vm.active = index;
                vm.onmenuclose($event,vm);
            }
            vm.menucloseother = function ($event) {
                vm.menucloseleft($event);
                vm.menucloseright($event);
                vm.onmenuclose($event,vm);
            }
        },
        ...
        collectData: function (vm, elem) {
            vm.tabs.push({
                ...
                iconshow:!!elem.getAttribute("iconshow"),
                iconclass:!!elem.getAttribute("iconclass")
            });
        ...
        $ready: function (vm, elem, vs) {
            ...
            avalon(elem).addClass("aoyou-tab aoyou-widget aoyou-widget-content" +
                    (vm.event === "click" ? " aoyou-tab-click" : "") + (vm.dir === "v" ?
                    " aoyou-tab-vertical" : "") + (vm.dir != "v" && vm.uiSize === "small" ? " aoyou-tab-small" : ""))
            // tab列表
...
/*
 <aoyou:tabs class="tabs-positive tabs-icon-only">
