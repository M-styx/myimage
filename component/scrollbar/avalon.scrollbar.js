 ...
 * @enName scrollbar
 * @introduce
 *  <p> 自定义滚动条样式，绑定ms-widget="scrollbar"的元素内必须包含一个class="aoyou-scrollbar-scroller"的视窗元素</p>
 */
define(["avalon", "text!./avalon.scrollbar.html", "../draggable/avalon.draggable", "css!./avalon.scrollbar.css"], function(avalon, template) {
    ...
    avalon.component("aoyou:scrollbar", {
        $template: template,
        keepReplace: true,
        ...
        //@config aoyounit(vm, options, vms) 完成初始化之后的回调,call as elem's method
        aoyounit: avalon.noop,
        ...
        $init: function(vm, elem) {
            ...
            vm.$draggableOpts = {
                ...
                drag: function(e, data) {
                    ...
                    vm._computer(function(obj) {
                        ...
                        return a
                    }, dr.attr("aoyou-scrollbar-index"), dr.attr("aoyou-scrollbar-pos"))
                }, 
            ...
            vm.$draggableOpts.stop = function(e, data) {
                ...
                vm.dragging = false
                avalon(data.element).removeClass("aoyou-state-active")
            }
            ...
            vm._show = function(e, always, index) {
                ...
                if(item) {
                    clearTimeout(item.data("aoyou-scrollbar-hidetimer"))
                    item.css("visibility", "visible")
                    item.css("opacity", 1)
                    if(!always) {
                        item.data("aoyou-scrollbar-hidetimer", setTimeout(function() {
                            item.css("opacity", 0)
                        }, 1000))
                    }
                }
            ...
            vm.update = function(ifInit, x, y) {
                ...
                avalon.each(vm._position, function(i, item) {
                    var bar = bars[i],
                        ...
                        dragger
                    if(bar) {
                        dragger = avalon(getByClassName("aoyou-scrollbar-dragger", bar.elem)[0])
                    }
                    // 拖动逻辑前移，确保一定是初始化了的
                    if(ifInit && dragger) {
                        dragger.attr("ms-draggable", "$,$draggableOpts")
                        dragger.attr("aoyou-scrollbar-pos", item)
                        dragger.attr("aoyou-scrollbar-index", i)
                    ...
                    if(!barDictionary[item]) {
                        if(bar) {
                            ...
                            bar.data("aoyou-scrollbar-needed", false)
                        }
                        return
                    } else {
                        if(bar) {
                            bar.data("aoyou-scrollbar-needed", true)
                    ...
                    if(bar) {
                        var sh = strToNumber(bar.css("height")),
                            ...
                            bh = sh,
                            bw = sw,
                            draggerpar = avalon(getByClassName("aoyou-scrollbar-draggerpar", bar[0])[0]),
                        ...
                        if(vm.showBarHeader) {
                            if(y == 0 && isVertical || !isVertical && x == 0) {
                                avalon(getByClassName("aoyou-scrollbar-arrow-up", bar[0])[0]).addClass("aoyou-state-disabled")
                            } else {
                                avalon(getByClassName("aoyou-scrollbar-arrow-up", bar[0])[0]).removeClass("aoyou-state-disabled")
                            }
                            if(y >= draggerpar.innerHeight() - dragger.innerHeight() && isVertical || !isVertical && x >= draggerpar.innerWidth() - dragger.innerWidth()) {
                               !vm.breakOutCallback && avalon(getByClassName("aoyou-scrollbar-arrow-down", bar[0])[0]).addClass("aoyou-state-disabled")
                            } else {
                                avalon(getByClassName("aoyou-scrollbar-arrow-down", bar[0])[0]).removeClass("aoyou-state-disabled")
                            }
                        }
                    }
                })
            ...
            vm._arrDown = function($event, diretion, position, barIndex,ismouseup) {
                ...
                if(ismouseup || ele.hasClass("aoyou-state-disabled")) {
                    return ele.removeClass("aoyou-state-active")
                }
                // 延时开启循环
                ele.data("setTimer", setTimeout(function(){
                    ele.addClass("aoyou-state-active")
            ...
            vm._barClick = function(e, position, barIndex) {
                if(vm.disabled) return
                var ele = avalon(this)
                if(ele.hasClass("aoyou-scrollbar-dragger")) return
                vm._computer(function(obj) {
                    return {
            ...
            vm._computer = function(axisComputer, barIndex, position, callback, breakOutCallbackCannotIgnore) {
                if(vm.disabled) return
                var bar = bars[barIndex]
                if(bar && bar.data("aoyou-scrollbar-needed")) {
                    var obj = {},
                        isVertical = position.match(/left|right/g)
                    obj.dragger = avalon(getByClassName("aoyou-scrollbar-dragger", bar[0])[0])
                    ...
                    obj.up = avalon(getByClassName("aoyou-scrollbar-arrow-up", bar[0])[0])
                    obj.down = avalon(getByClassName("aoyou-scrollbar-arrow-down", bar[0])[0])
                    ...
                    if(isVertical) {
                        if(xy.y < 0) {
                            xy.y = 0
                            obj.up.addClass("aoyou-state-disabled")
                            breakOut = ["v", "up"]
                        } else {
                            obj.up.removeClass("aoyou-state-disabled")
                        }
                        if(xy.y > obj.draggerparHeight - obj.draggerHeight) {
                            xy.y = obj.draggerparHeight - obj.draggerHeight
                            breakOut = ["v", "down"]
                            obj.down.addClass("aoyou-state-disabled")
                        } else {
                            obj.down.removeClass("aoyou-state-disabled")
                        }
                        ...
                        if(xy.x > obj.draggerparWidth - obj.draggerWidth) {
                            ...
                            breakOut = ["h", "down"]
                            // 有溢出检测回调，不disable
                            !vm.breakOutCallback && obj.down.addClass("aoyou-state-disabled")
                        } else {
                            obj.down.removeClass("aoyou-state-disabled")
                        }
            ...
            vm._draggerDown = function(e, isdown) {
                ...
                if(isdown) {
                    ...
                    ele.removeClass("aoyou-state-active")
                }
            }
        ...
        $ready: function(vm, elem) {
            ...
            avalon.each(children, function(i, item) {
                var ele = avalon(item)
                if(ele.hasClass("aoyou-scrollbar") || ele.hasClass("ui-scrollbar")) {
                    bars.push(ele)
                } else if(ele.hasClass("aoyou-scrollbar-scroller") || ele.hasClass("ui-scrollbar-scroller")) {
                    vm.setScroller(ele)
                }
            })
            ...
            if(vm.position.match(/left|right/g)) {
                ...
                function wheelLike(diretion, arr, e, func) {
                    avalon.each(arr, function(i, item) {
                        if(!bars[i].data("aoyou-scrollbar-needed")) return
