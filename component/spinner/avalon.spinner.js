 ...
 */
define(["avalon", "text!./avalon.spinner.html",  "css!./avalon.spinner.css"], function(avalon, template) {
    var _interface = function () {}

    avalon.component("aoyou:spinner", {
        // 内部变量
        _valueIsValid: true,
        _correctValue: 0,
        _normalstyle:true,
        ...
        $init: function (vm) {
            if(vm.side){
                if(vm.side !='center'){
                    vm._normalstyle = false;
                    if(vm.side=='left'){
                        vm._leftstyle = true;
                    }else if(vm.side=='right'){
                        vm._rightstyle = true;
                    }
                }
            }
            ...
            vm._validate = function(val) {
                ...
                if(val === "-" || String(val).trim() === ""){
                    ...
                    if(vm.spdecimal==false){
                        vm._correctValue = parseInt(val)
                        vm.value = parseInt(val)
                    }
                }
            }
        },
